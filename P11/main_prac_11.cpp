...
void animate(void)
{
	if (play) {
		...
		if (estado == 1) {
			...
			if (movKit_x < 180) {
				...
				rotKit_y = 180; //para que no regrese 
				estado = 2;
			}
		}
		if (estado == 2) {
 ...
 if (reversa) {
	  ...
	  if (estado == 0 ) { //ESTADO A reversa

		  if (rotKit_y == 0) rotKit_y = 180;
		  if (movKit_z > -150) { 
			  movKit_z -= 1.0f; 
		  }
          else { // Transición al edo. B
	  ...
	  if (estado == 1) {
		  ...
		  if (rotKit_y == 90) rotKit_y = 270;
		  if (movKit_x > 0) {
			  movKit_x -= 1.0f;
		  }
		  else { // Transición al edo. C
			  rotKit_y = 180;
			  estado = 0;
		  }
	  }
